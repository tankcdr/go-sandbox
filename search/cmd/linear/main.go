...
func timeLinearSearch(slice []int, target int) (index int, numTessts int, runTime time.Duration) {
	startTime := time.Now()

	index, numTests := search.LinearSearch(slice, target)
...
func main() {
	...
	for ok := true; ok; {
		...
		index, numTests, runTime := timeLinearSearch(slice, target)
